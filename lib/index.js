...
import { recordFromLog, DEFAULT_LEDGER_LIMIT, isOwnInjectedSource } from './script-entry.js'
...
function createConfigEditorScope(ctx, ns, readCurrent) {
  ...
  /**
   * 惰性取 configEditor（每次写入前重取）。
   *
   * 为什么不在 apply 时缓存实例：configEditor 由宿主异步注册，插件 apply 的
   * 那一刻它可能还没就绪；若在这里缓存一个 undefined，设置写入就永远失败了。
   * 每写一次前现取，保证「扫码落地发生在服务就绪之后」也能命中。
   */
  const editorOf = () => {
    try {
      const editor = typeof ctx.get === 'function' ? ctx.get('configEditor') : undefined
      return editor && typeof editor?.entries === 'function' && typeof editor?.edit === 'function'
        ? editor
        : undefined
    } catch {
      return undefined
  ...
  return {
    ...
    async update(patch) {
      ...
      if (!editor) throw new Error('设置命名空间不可用（configEditor 未就绪）')
      const entry = entryOf(editor)
...
export function apply(ctx, config) {
  ...
  if (typeof ctx.settings?.register === 'function') {
    ...
    // configEditor 由宿主异步注册；scope 内部每次写入前惰性取，apply 时只探测
    // 「ctx 有没有取服务的能力」——没有（精简/裸环境）才退回静态配置。
    // 注意只能用 ctx.get()（读 store、不要求 inject）：直接读 ctx.configEditor
    // 会对「未 inject 的服务」抛 "cannot get property ... without inject"，
    // 让 apply() 在精简宿主下一启动就崩。
    settingsScope = typeof ctx.get === 'function' ? createConfigEditorScope(ctx, name, () => activeConfig) : null
    if (settingsScope) {
      settingsLive = true
      ctx.effect(() => settingsScope.watch((next) => {
        activeConfig = next
  ...
  const qqIm = installQqIm(ctx, {
    ...
    // 建会话失败且报 Unknown agent preset 时，先把预设注册进宿主再重试
    // （DSH 升级后 create 强校验 agentPresets registry，见 registerPresetToHost）。
    registerPreset: (presetId) => registerPresetToHost(presetId),
  ...
  /**
   * 从某预设的 story.json 读取完整人设（若存在）。
   *
   * ## 为什么需要（线上症状：角色只写故事不发言）
   *
   * 人设的**权威副本**存在预设里（`~/.dsh/.agent-presets/<id>/story.json`，
   * 设置页「保存预设」时写入的完整 story）。而配置里的 `story` 字段（全局
   * 与 per-bot）可能只是 schema 默认的空壳（`Unnamed character` + 空字段）——
   * 例如扫码落地 / 配置写入时用空 story 覆盖过。此时若只读配置 story，
   * pre-step 注入的就是空壳人设，模型不知道角色是谁、不知道要调 interlude_say，
   * 于是「只写故事不发言」。
   *
   * 所以当配置 story 明显是空壳时，回退到预设的 story.json（完整人设）。
   *
   * @param {string} presetId 预设 id（如 preset-hds-xxx）。
   * @returns {object|undefined} 预设里的完整 story；读不到返回 undefined。
   */
  const storyFromPreset = (presetId) => {
    const dir = presetDirOf(presetId)
    if (!dir) return undefined
    try {
      const raw = fs.readFileSync(path.join(dir, 'story.json'), 'utf8')
      const parsed = JSON.parse(raw)
  ...
  /**
   * 判断一个 story 是不是「空壳」（没有任何真实人设内容）。
   *
   * 空壳 = 主角名是默认占位（Unnamed character 或空）且关键描述字段全空。
   * 用它来决定要不要回退到预设 story。
   *
   * @param {object|undefined} story
   * @returns {boolean}
   */
  const isShellStory = (story) => {
    if (!story || typeof story !== 'object' || Array.isArray(story)) return true
    const name = String(story.character?.name ?? '').trim()
    const filled = String(story.character?.profile ?? '').trim()
      || String(story.character?.speech ?? '').trim()
      || String(story.perspective ?? '').trim()
      || String(story.plot?.startingPoint ?? '').trim()
    return !name || name === 'Unnamed character' ? !filled : false
  }

  /**
   * 一个 bot 的生效人设：bot 自己配了非空 `story` 用它，否则继承全局 `config.story`；
   * 两者都只是空壳时，回退到该 bot 的 agentPreset 预设里的完整 story。
   *
  ...
  const storyForBot = (bot) => {
    const own = bot?.story
    if (!isShellStory(own)) return own
    const globalStory = current().story
    if (!isShellStory(globalStory)) return globalStory
    const presetStory = storyFromPreset(bot?.agentPreset)
    if (presetStory) return presetStory
    return own && typeof own === 'object' && !Array.isArray(own) ? own : globalStory
  }
  ...
  /**
   * 把本插件自建的角色预设注册进宿主 `agentPresets` 服务。
   *
   * ## 为什么需要（线上症状：QQ 收得到消息却永远不回复）
   *
   * DSH 升级后，`sessionController.create({ agentPreset })` 会**强校验**预设必须
   * 注册在宿主的 `agentPresets` registry（`dsh-agent-preset-registry`）里；未注册
   * 的 id 直接抛 `Unknown agent preset`。本插件的预设只写在
   * `~/.dsh/.agent-presets/<id>/`（`preset.yml` + `agent.cordis.yml` + `story.json`）——
   * **宿主从不扫描这个目录**，因此 `create` 失败 → 降级建出无预设会话 →
   * `isRoleplaySession` 判定失败 → pre-step 不注入人设/发言规则 → 模型不知道
   * `interlude_say` → 消息 delivered 却没有回复（出站测试走绑定+通道，不受影响，
   * 所以「测试能投、用户消息无反应」）。
   *
   * 修复：把 `agent.cordis.yml`（本插件自己生成的插件列表）解析成宿主要求的
   * `plugins` 数组，调用 `agentPresets.register({ id, name, plugins })`。
   *
   * ## 幂等与降级
   *
   * - 宿主 `register` 对重复 id 抛 `Duplicate agent preset`——这里先查已有定义，
   *   重复则静默跳过（不改写宿主已挂载的版本）；
   * - 宿主没有 `agentPresets` 服务（老版本 / 精简宿主）时**静默跳过**，行为与
   *   升级前一致（旧版宿主不校验 registry）；
   * - 解析或注册失败只记 warn，**绝不影响插件其余功能**。
   *
   * @param {string} id 预设 id（如 `preset-hds-xxx`）。
   * @returns {Promise<boolean>} 是否已在宿主注册（含已存在的幂等命中）。
   */
  const registerPresetToHost = async (id) => {
    const registry = (() => {
      try { return typeof ctx.get === 'function' ? ctx.get('agentPresets') : ctx.agentPresets } catch { return undefined }
    })()
    if (!registry || typeof registry?.register !== 'function') {
      // 宿主不提供预设注册服务：旧版行为，无需也不可注册。
      return false
    }
    const dir = presetDirOf(id)
    if (!dir) return false
    const cordisFile = path.join(dir, 'agent.cordis.yml')
    if (!fs.existsSync(cordisFile)) return false

    // 已注册（幂等命中）：宿主 definitions 里有该 id 就直接算成功。
    try {
      const existing = typeof registry.resolve === 'function' ? await registry.resolve(id) : undefined
      if (existing && existing.id === id) return true
    } catch { /* 不 resolve 也不拦（resolve 对未知 id 抛 Unknown） */ }

    let plugins
    try {
      const doc = yaml.load(fs.readFileSync(cordisFile, 'utf8'))
      plugins = Array.isArray(doc) ? doc : []
    } catch (error) {
      warn(`解析角色预设 ${id} 的插件列表失败，未注册到宿主：${error?.message ?? String(error)}`)
      return false
    }
    ...
    try {
      ...
      if (m?.[1]) name = String(m[1]).trim().replace(/^["']|["']$/g, '') || id
    } catch { /* 没有 preset.yml 就用 id 作名 */ }

    try {
      await registry.register({ id, name, plugins })
      info(`已向宿主注册角色预设 ${id}（${name}）`)
      return true
    } catch (error) {
      // Duplicate：别的实例/旧机制已注册过，视为成功。
      if (/Duplicate agent preset/i.test(String(error?.message ?? error))) return true
      warn(`注册角色预设 ${id} 到宿主失败：${error?.message ?? String(error)}`)
      return false
    }
  }

  /**
   * 启动/配置变更后把**所有**自建预设注册进宿主（fire-and-forget）。
   *
   * 只在宿主提供 `agentPresets` 服务时做；失败只记日志，不阻塞启动。
   * 返回的 Promise 由调用方 `void` 掉。
   */
  const registerAllPresetsToHost = async () => {
    try {
      const ids = readPresetsJson()
        .filter((p) => typeof p?.id === 'string' && p.id.startsWith('preset-hds-'))
        .map((p) => p.id)
      await Promise.all([...new Set(ids)].map((id) => registerPresetToHost(id).catch(() => false)))
    } catch (error) {
      warn(`批量注册角色预设到宿主失败：${error?.message ?? String(error)}`)
    }
  }
    ...
    if (state.canonInjected !== true || formChanged) {
      ...
      if (state.canonInjected !== true) {
        ...
        if (canonText) {
          messages.push(createUserMessage({
            ...
            source: { kind: name, plugin: name, form: 'snapshot', sections: [{ name: `${name}:canon`, text: canonText }] },
          }))
        }
      } else {
        ...
        messages.push(createUserMessage({
          ...
          source: { kind: name, plugin: name, form: 'snapshot', sections: [{ name: `${name}:im-form` }] },
        }))
      }
      ...
      if (rulesText) {
        messages.push(createUserMessage({
          ...
          source: { kind: name, plugin: name, form: 'snapshot', sections: [{ name: `${name}:rules`, text: rulesText }] },
        }))
      }
      if (imActive !== 'im') {
        ...
        messages.push(createUserMessage({
          ...
          source: { kind: name, plugin: name, form: 'snapshot', sections: [{ name: `${name}:story`, text: storyText }] },
        }))
      }
    ...
    if (stepNo === 1) {
      ...
      if (text) {
        ...
        messages.push(createUserMessage({
          ...
          source: { kind: name, plugin: name, form: 'snapshot', sections: [{ name, text }] },
        }))
      }
    }
        ...
        case 'advance': {
          ...
          try {
            agent.followup(createUserMessage({
              ...
              source: { kind: name, plugin: name, form: 'notice', summary: '手动推进一次剧本' },
            }))
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (type === 'user/message') {
      ...
      // 判据见 isOwnInjectedSource：必须同时认新形态（kind=插件名，会话格式 v4 要求）
      // 与旧形态（kind='plugin'）——写死 'plugin' 会让防护失效（详见该函数注释）。
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (event?.type === 'user/message') {
      ...
      // 宿主在回合开始时把 followup 的消息以 `user/message` 落进会话（source
      // 由本插件署名），若不排除它，标记会在 assistant 发言之前就被清掉，
      // 于是同一段 interlude_say 又会被交互式路径收一遍——线上「每条发两遍」原样复现。
      //
      // 判据必须用 isOwnInjectedSource：会话格式 v4 拒绝 kind='plugin'（只用
      // 插件名），写死 'plugin' 会被这行判成「真人说话」而清掉标记——这正是
      // 「每条发两遍」复现的根因。
      const source = event?.data?.source
      if (!isOwnInjectedSource(source)) pluginDrivenTurns.delete(key)
      return
    }
    ...
    if (binding && service) {
      ...
      try {
        agent.followup(createUserMessage({
          ...
          source: { kind: name, plugin: name, form: 'notice', summary: `幕间到点：${target.summary}` },
        }))
      } catch (error) {
      ...
      if (!delivered.ok && cfg.im?.keepPassive === true) {
        ...
        agent.followup(createUserMessage({
          content: [{ type: 'text', text: renderProactiveText(target, now, zone, state) }],
          source: { kind: name, plugin: name, form: 'notice', summary: `幕间到点（IM 投递失败）：${target.summary}` },
        }))
      }
      saveState(key, state)
      return true
  ...
  function repetitionGuardFor(state, payload, cfg) {
    ...
    // 本轮有没有真实用户消息（插件注入不算；判据同 isOwnInjectedSource，
    // 必须认 kind=插件名，否则插件唤起的回合会被当成真人说话）。
    const incoming = Array.isArray(payload?.messages) ? payload.messages : []
    const hasUserMessage = incoming.some((message) => {
      const source = message?.data?.source ?? message?.source
      if (isOwnInjectedSource(source)) return false
  ...
  // 把本插件自建的角色预设注册进宿主 agentPresets（DSH 升级后 create 强校验
  // registry，未注册的 preset 会让自动建会话失败 → 收得到消息却永远不回复）。
  // fire-and-forget：注册失败只记日志，不阻塞启动；拿不到 agentPresets 服务
  // （老宿主/精简宿主）则静默跳过，行为与升级前一致。
  Promise.resolve().then(() => registerAllPresetsToHost()).catch(() => {})
