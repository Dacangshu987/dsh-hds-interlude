...
/** 本插件的生产者名（与 `state.js` 的 `PLUGIN_NAME` 同值；此处再导出避免循环依赖）。 */
export const PRODUCER_NAME = 'hds-interlude'

/**
 * 判断一条 `user/message` 的 source 是不是**本插件自己注入的**（不是真人说话）。
 *
 * ## 为什么需要一个共享判据（线上事故：QQ 里每条消息都发两遍）
 *
 * 这套判断原先散落在 5 处，且写死了 `source.kind === 'plugin'`：
 *   - `index.js`  的 `pluginDrivenTurns` 标记（决定交互式路径要不要重复投递）；
 *   - `index.js`  的 `pendingTurns` / `repetitionGuardFor`；
 *   - `script-entry.js` 的账本（插件注入不该记成「用户说过」）；
 *   - `state.js`  的 `applyEvent`（插件注入不该更新 `lastUserAt`）。
 *
 * 但会话格式 v4 **拒绝** `kind === 'plugin'`（retired syntax，只接受
 * producer-owned kind），所以注入消息的 `kind` 改成了插件自己的名字
 * （`PRODUCER_NAME`）。写死 `'plugin'` 的判据于是全部失效：
 * 插件唤起的回合被误判成「真人说话」→ `pluginDrivenTurns` 标记被提前清掉
 * → 同一段 `interlude_say` 又被交互式路径收一遍 → **每条发两遍**。
 *
 * 所以判据收敛到这里一处，并同时认两种形态：
 *   - 新形态：`kind === PRODUCER_NAME`（会话格式 v4 要求）；
 *   - 旧形态：`kind === 'plugin'` + `plugin === PRODUCER_NAME`（历史日志/兼容）。
 *
 * 只认**自己的**注入：别家插件的 `'plugin'` source（`plugin` 不是本插件名）
 * 不算，避免替别人的系统提示记账或吞掉别人注入的真实反馈。
 *
 * @param {object|undefined} source 事件的 `data.source`（或 message.source）。
 * @returns {boolean} true = 本插件注入。
 */
export function isOwnInjectedSource(source) {
  if (!source || typeof source !== 'object') return false
  const kind = source.kind
  const plugin = source.plugin
  if (kind === PRODUCER_NAME) return true
  if (kind === 'plugin' && plugin === PRODUCER_NAME) return true
  return false
}
...
export function draftFromEvent(event, { participantId = '', actor = '' } = {}) {
  ...
  if (type === 'user/message') {
    ...
    if (isOwnInjectedSource(source)) return undefined
